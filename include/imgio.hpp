...
public:
	...
	void write_image(const std::string file_name, T* data_,std::string out_t){
		...
		if(out_t == "pgm"){
			...
			this->writeCostMap ((fname+"png").c_str(),data_);
		}
		else{
  ...
  inline bool isValid (float* data_,const int32_t u,const int32_t v) {
    ...
    void writeDisparityMap (const char* file_name,float* data_) {
	...
	template<typename T>
	void writeCostMap(const char* file_name,T * buffer){
		png::image<png::gray_pixel_16> img(this->width,this->height);
	  	#pragma omp parallel
	  	{
		  #pragma omp for
		  for(uint i=0;i<img.get_height();i++)
